...
*Setup instructions for a Gitian build of Octocoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Octocoin
...
- [Building Octocoin](#building-octocoin)
...
Clone the git repositories for octocoin and Gitian and then checkout the octocoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the octocoin repository
...
To build Octocoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the octocoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/octocoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/octocoin-project/octocoin
...
For example:
```bash
URL=https://github.com/user/octocoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit octocoin=${COMMIT} --url octocoin=${URL} ../octocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit octocoin=${COMMIT} --url octocoin=${URL} ../octocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit octocoin=${COMMIT} --url octocoin=${URL} ../octocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/octocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/octocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/octocoin-build.assert
```
